//! shell-init / _shell-start / _detect-workspace / completions: snapshot the
//! emitted shell text and drive real shells against it.
...
fn shell_init(shell: &str) -> String {
    let env = TestEnv::new();
    let out = env.run(&["shell-init", shell]);
...
fn completions_zsh_starts_with_compdef() {
    let env = TestEnv::new();
    let out = env.run(&["completions", "zsh"]);
    let cap = capture(&env, &out);
...
fn shell_init_zsh_is_valid_zsh() {
    let env = TestEnv::new();
    let out = env.run(&["shell-init", "zsh"]);
    let cap = capture(&env, &out);
    assert_eq!(cap.exit, 0);
...
fn shell_init_bash_is_valid_bash() {
    let env = TestEnv::new();
    let out = env.run(&["shell-init", "bash"]);
    let cap = capture(&env, &out);
    assert_eq!(cap.exit, 0);
...
fn dynamic_workspace_completion_lists_real_workspaces() {
    ...
    let _ = env.run(&["create", "feat-b"]);
    let _ = env.run(&["create", "feat-c"]);
...
fn shell_start_no_tmux_emits_cd_and_exports() {
    let env = setup_workspace_for_start();
    let out = env.run(&["_shell-start", "feat-a", "--no-tmux"]);
    let cap = capture(&env, &out);
...
fn shell_start_missing_workspace_exits_nonzero() {
    let env = TestEnv::new();
    let out = env.run(&["_shell-start", "ghost", "--no-tmux"]);
    let cap = capture(&env, &out);
...
fn detect_workspace_finds_root_from_subdir() {
    ...
    let out = env.run(&["_detect-workspace", sub.to_str().unwrap()]);
    let cap = capture(&env, &out);
    assert_eq!(cap.exit, 0);
    assert!(
...
fn detect_workspace_outside_workspaces_dir_emits_nothing() {
    let env = TestEnv::new();
    let out = env.run(&["_detect-workspace", env.tmp.path().to_str().unwrap()]);
    let cap = capture(&env, &out);
    assert_eq!(cap.exit, 0);
...
fn list_workspaces_prints_names_one_per_line() {
    ...
    let out = env.run(&["_list-workspaces"]);
    let cap = capture(&env, &out);
    assert_eq!(cap.exit, 0);
