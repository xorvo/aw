...
function render() {
    const list = $('#list');
    if (!sessions.length) {
        list.innerHTML = '<div class="empty">no active agent sessions</div>';
        $('#hdrdot').className = 'dot idle';
        return;
    }
    ...
    for (const id of now) {
        if (!lastAttn.has(id)) {
            const s = sessions.find(x => x.pane_id === id);
            notify(s);
        }
    }
    lastAttn = now;
...
function xterm256(n) {
    ...
    if (n < 232) {
        n -= 16;
        const f = (v) => [0, 95, 135, 175, 215, 255][v];
...
function ansiToHtml(input) {
    ...
    const span = () => {
        ...
        if (under)
            st.push('text-decoration:underline');
        if (st.length) {
        ...
        for (let i = 0; i < codes.length; i++) {
            ...
            if (c === 0) {
                ...
                const set = (v) => { if (c === 38)
                    ...
                    bg = v; };
                if (codes[i + 1] === 5) {
                    set(xterm256(codes[i + 2] || 0));
                    i += 2;
                }
                else if (codes[i + 1] === 2) {
                    set('rgb(' + (codes[i + 2] || 0) + ',' + (codes[i + 3] || 0) + ',' + (codes[i + 4] || 0) + ')');
                    i += 4;
                }
            }
        }
...
function showSheet(pane) {
    ...
    pendingScreen = null;
    refreshScreen();
    openScreenStream(pane);
    if (fitMode) {
        lastFit = '';
        setTimeout(applyFit, 150);
    } // fit the newly-opened session
}
function hideSheet() { try {
    saveDraftNow();
}
catch { } if (fitMode && current)
    unfit(current); $('#sheet').classList.remove('open'); current = null; closeScreenStream(); try {
    kbd.blur();
}
catch { } try {
    draft.blur();
...
function openSheet(pane) {
    history.pushState({ pane }, '', '#' + encodeURIComponent(pane));
    showSheet(pane);
}
...
function applyScreen(screen) {
    ...
    if (screen === lastScreen)
        return; // unchanged -> no DOM work, no scroll jump
    lastScreen = screen;
    const t = $('#term');
...
function openScreenStream(pane) {
    ...
    screenES.onmessage = e => { try {
        applyScreen(JSON.parse(e.data));
    }
    catch { } };
    // EventSource auto-reconnects on transient errors
}
function closeScreenStream() { if (screenES) {
    screenES.close();
    screenES = null;
} }
// one-shot fetch for instant echo right after a local keystroke
async function refreshScreen() {
    if (!current || scrInFlight)
        return;
    scrInFlight = true;
    try {
        const r = await fetch('/api/screen?pane=' + encodeURIComponent(current) + '&lines=80');
        const j = await r.json();
        applyScreen(j.screen || '');
    }
    catch { }
    finally {
        scrInFlight = false;
...
async function key(k) { if (!current)
    return; await post({ pane: current, key: k }); liveRefresh(); }
...
kbd.addEventListener('keydown', e => {
    if (e.isComposing)
        return; // let IME finish
    const k = SPECIAL[e.key];
    if (k) {
        e.preventDefault();
        key(k);
    } // named keys never become local text
});
kbd.addEventListener('input', () => {
    const v = kbd.textContent;
    if (!v || !current) {
        kbd.textContent = '';
        return;
    }
    kbd.textContent = '';
    post({ pane: current, text: v });
    liveRefresh();
});
...
let kbOpen = false;
$('#kbBtn').addEventListener('pointerdown', e => { if (kbOpen)
    e.preventDefault(); }); // keep focus so we can blur
$('#kbBtn').addEventListener('click', () => { if (!current)
    return; if (kbOpen)
    kbd.blur();
else
    kbd.focus(); });
$('#term').addEventListener('click', () => { if (current)
    kbd.focus(); });
...
function loadDraft() { if (current)
    draft.value = LS.getItem(wsKey()) || ''; }
function saveDraftNow() { if (current)
    try {
        LS.setItem(wsKey(), draft.value);
    }
    catch { } }
let dft;
...
function hideEditor() { if (!editorOpen)
    return; saveDraftNow(); try {
    draft.blur();
}
catch { } editor.classList.remove('open'); editorOpen = false; }
function openEditor(fromPop) {
    if (!current)
        return;
    if (!fromPop)
        history.pushState({ pane: current, editor: 1 }, '', '#' + encodeURIComponent(current));
    showEditor();
}
...
function termCell() {
    const t = $('#term'), cs = getComputedStyle(t);
...
async function applyFit() {
    if (!fitMode || !current)
        return;
    const { cols, rows } = termCell(), k = cols + 'x' + rows;
    if (k === lastFit)
        return;
    lastFit = k;
    try {
        await fetch('/api/resize', { method: 'POST', headers: { 'content-type': 'application/json' },
            body: JSON.stringify({ pane: current, cols, rows }) });
        liveRefresh();
    }
    catch { }
}
async function unfit(pane) {
    lastFit = '';
    if (!pane)
        return;
    try {
        await fetch('/api/unfit', { method: 'POST', headers: { 'content-type': 'application/json' },
...
$('#fitBtn').addEventListener('click', () => {
    if (!current)
        return;
    fitMode = !fitMode;
    LS.setItem('aw:fit', fitMode ? '1' : '0');
    $('#fitBtn').classList.toggle('on', fitMode);
    if (fitMode) {
        applyFit();
        toast('fit to screen — note: also resizes on your Mac');
    }
    else {
        unfit(current);
        toast('size restored');
...
addEventListener('pageshow', () => { if (fitMode && current) {
    lastFit = '';
    setTimeout(applyFit, 200);
} });
document.addEventListener('visibilitychange', () => { if (document.visibilityState === 'visible' && fitMode && current) {
    lastFit = '';
    setTimeout(applyFit, 200);
} });
