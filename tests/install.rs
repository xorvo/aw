...
fn install_shell_writes_zshrc_with_marker() {
    let env = TestEnv::new();
    let cap = capture(&env, &env.run(&["install", "shell", "--shell", "zsh"]));
...
fn install_shell_idempotent() {
    let env = TestEnv::new();
    let _ = env.run(&["install", "shell", "--shell", "zsh"]);
    let _ = env.run(&["install", "shell", "--shell", "zsh"]);
...
fn install_tmux_bindings_writes_block() {
    let env = TestEnv::new();
    let cap = capture(&env, &env.run(&["install", "tmux-bindings"]));
    assert_eq!(cap.exit, 0, "{}", cap.stderr);
...
fn install_tmux_bindings_replaces_block_in_place() {
    ...
    .unwrap();
    let _ = env.run(&["install", "tmux-bindings"]);
...
fn install_tmux_bindings_honors_xdg_config_home_env() {
    ...
    let mut cmd = std::process::Command::new(common::aw_bin());
...
fn install_claude_hooks_writes_all_events() {
    let env = TestEnv::new();
    let cap = capture(&env, &env.run(&["install", "hooks", "--agent", "claude"]));
    assert_eq!(cap.exit, 0, "{}", cap.stderr);
...
fn install_claude_hooks_idempotent_and_preserves_other_entries() {
    ...
    .unwrap();
    let _ = env.run(&["install", "hooks", "--agent", "claude"]);
    let _ = env.run(&["install", "hooks", "--agent", "claude"]);
...
fn install_codex_hooks_writes_hooks_and_enables_feature() {
    let env = TestEnv::new();
    let cap = capture(&env, &env.run(&["install", "hooks", "--agent", "codex"]));
    assert_eq!(cap.exit, 0, "{}", cap.stderr);
    ...
    assert!(toml.contains("hooks = true"), "{}", toml);
}
...
fn install_pi_writes_extension_files() {
    let env = TestEnv::new();
    let cap = capture(&env, &env.run(&["install", "hooks", "--agent", "pi"]));
    assert_eq!(cap.exit, 0, "{}", cap.stderr);
...
fn install_all_runs_every_step() {
    let env = TestEnv::new();
    let cap = capture(&env, &env.run(&["install", "all"]));
    assert_eq!(cap.exit, 0, "{}", cap.stderr);
