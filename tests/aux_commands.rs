//! Auxiliary commands: edit-config, edit-base, open-home, sync.
//!
...
use common::{aw_bin, capture, fixtures, TestEnv};

#[test]
fn edit_config() {
    let env = TestEnv::new();
    let cap = capture(&env, &env.run(&["edit-config"]));
    assert_eq!(cap.exit, 0, "edit-config: {}", cap.stderr);
}

#[test]
fn edit_base_existing() {
    let env = TestEnv::new();
    assert_eq!(capture(&env, &env.run(&["init"])).exit, 0);
    let cap = capture(&env, &env.run(&["edit-base", "default"]));
    assert_eq!(cap.exit, 0, "edit-base: {}", cap.stderr);
}

#[test]
fn edit_base_missing() {
    let env = TestEnv::new();
    let cap = capture(&env, &env.run(&["edit-base", "ghost"]));
...
fn open_home() {
    let env = TestEnv::new();
    let cap = capture(&env, &env.run(&["open-home"]));
...
fn run_sync(env: &TestEnv, workspace: &Path) -> (String, i32) {
    let path = format!(
        "{}:/opt/homebrew/bin:/usr/local/bin:/usr/bin:/bin",
        env.fake_bin.display()
    );
    let out = std::process::Command::new(aw_bin())
        .args(["sync"])
...
fn sync_fixture() -> (TestEnv, PathBuf, PathBuf) {
    let env = TestEnv::new().with_fake_remote("repo1");
    let cfg = fixtures::config_with_one_remote(&env, "repo1");
    let env = env.with_config(&cfg);
    assert_eq!(capture(&env, &env.run(&["init"])).exit, 0);
    assert_eq!(capture(&env, &env.run(&["create", "feat-sync"])).exit, 0);
    let workspace = env.workspaces_dir.join("feat-sync");
    let repo = workspace.join("repo1");
    (env, workspace, repo)
}

// Remote and local in sync → "already up to date" + exit 0.
#[test]
fn sync_up_to_date() {
    let (env, workspace, _) = sync_fixture();
    let (stdout, exit) = run_sync(&env, &workspace);
...
#[test]
fn sync_outside_workspace() {
    let env = TestEnv::new();
    let cap = capture(&env, &env.run(&["sync"]));
    assert_ne!(cap.exit, 0, "sync outside should fail");
}

// ---- sync (fast-forward behavior) ----
//
// When the default branch is checked out, sync fast-forwards via
// `merge --ff-only` so the index/worktree move with the ref — a bare
// `update-ref` would leave a stale index, with upstream commits showing up
// inverted as staged changes. The not-checked-out path still uses
// `update-ref` and never touches the worktree.
...
#[test]
fn sync_ff_checked_out_updates_worktree() {
...
#[test]
fn sync_ff_checked_out_blocked_by_local_changes() {
    let (env, workspace, repo) = sync_fixture();
...
#[test]
fn sync_ff_not_checked_out_leaves_worktree_alone() {
    let (env, workspace, repo) = sync_fixture();
